...
const RecepItem = ({ route }) => {
  ...
  let currentRecept = recept.getRecipeById(receptId)
  ...
  const likeFunc = () => {
    setIsLiked()
    recept.setLiked(currentRecept)
  }
  return (
    <ScrollView style={{ padding: 10 }}>
      <Image source={{ uri: currentRecept.img }} style={{ width: "100%", height: 250, resizeMode: 'contain', borderRadius: 20 }} />
      ...
      <View style={{ display: 'flex', alignItems: 'center', marginTop: 20 }}>
        ...
        {currentRecept.ingredients.map(({ name, amount }) => (
      ...
      <View style={{ display: 'flex', alignItems: 'center', marginTop: 20 }}>
        ...
        <View style={{ padding: 30 }}>
          {currentRecept.cooking.map(({ step, description }) => (
          ...
          <Comments modalVisible={modalVisible} setModalVisible={setModalVisible} />
        </View>
      </View>
    </ScrollView>
  )
